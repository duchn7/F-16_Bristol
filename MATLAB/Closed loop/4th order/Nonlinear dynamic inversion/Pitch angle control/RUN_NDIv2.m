function RUN_NDIv2() % Code for theta pitch control, adapted to use ode45
    ...
    for i = 1:length(time_log)
        [~, dstab_log(i)] = eom_NDI(time_log(i), X(i,:)', last_dstab, theta_des);
    ...
    function dxdt = wrapper(t, x, theta_des)
        [dxdt, ~] = eom_NDI(t, x, last_dstab, theta_des); %embedded controller
    end
end
