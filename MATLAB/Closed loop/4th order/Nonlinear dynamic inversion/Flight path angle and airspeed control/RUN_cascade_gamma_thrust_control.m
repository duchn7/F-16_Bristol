function RUN_cascade_gamma_thrust_control()
...
% xxx = wrapper(0, x0, V_des ,gamma_des);
    % Call ODE solver
    [time_log, X] = ode45(@(t, x) wrapper(t, x, V_des ,gamma_des), tspan, x0);

    % Reconstruct control input dstab and thrust at each output time
    dstab_log = zeros(2,length(time_log));
    for i = 1:length(time_log)
        [~, dstab_log(:,i)] = eom_NDI_2input(time_log(i), X(i,:)', last_input, V_des ,compute_theta_des(X(i,:)', gamma_des));
    ...
    hold on
    yline(V_des,'r--', 'LineWidth', 1.5);
    legend('Actual V','Desired V')
    ...
    xlabel('Time [s]');
    ylabel('Stabilator Deflection [deg]');
    title('Control Input');
    
    subplot(3,1,3)
    plot(time_log, dstab_log(2,:), 'k', 'LineWidth', 1.5);
    xlabel('Time [s]');
    ylabel('Thrust [N]');
    title('Control Input');    

    % =============== Nested Function ==================
    function dxdt = wrapper(t, x, V_des,gamma_des)
        ...
        eps = 1e-3;
        dx1 = eom(t, x, [last_input + eps; 25; 10000]); % Assumed thrust fixed at this value, not totally accurate
        ...
        [dxdt, ~] = eom_NDI_2input(t, x, last_input, V_des,theta_des);
    end
