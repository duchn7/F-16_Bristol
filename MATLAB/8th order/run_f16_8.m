...
tic; [t,x] = ode45(@eom_8,[0 150],x0,options,u0); toc % MATLAB
% tic; [t,x] = ode45(@eom_8_mex,[0 150],x0,options,u0); toc % Accelerated

figure
subplot(3,3,1); plot(t,x(:,1)*180/pi,'k','LineWidth',1); ylabel('\alpha (deg)');
subplot(3,3,4); plot(t,x(:,2)*180/pi,'k','LineWidth',1); ylabel('\beta (deg)');
subplot(3,3,7); plot(t,x(:,3)       ,'k','LineWidth',1); ylabel('V (m/s)'); xlabel('t (s)');
subplot(3,3,2); plot(t,x(:,4)*180/pi,'k','LineWidth',1); ylabel('p (deg/s)'); 
subplot(3,3,5); plot(t,x(:,5)*180/pi,'k','LineWidth',1); ylabel('q (deg/s)');
subplot(3,3,8); plot(t,x(:,6)*180/pi,'k','LineWidth',1); ylabel('r (deg/s)');  xlabel('t (s)');
subplot(3,3,3); plot(t,x(:,7)*180/pi,'k','LineWidth',1); ylabel('\phi (deg)');
subplot(3,3,6); plot(t,x(:,8)*180/pi,'k','LineWidth',1); ylabel('\theta (deg)'); xlabel('t (s)');